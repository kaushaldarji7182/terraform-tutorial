...
Refer D:\PraiseTheLord\HSBGInfotech\Others\terraform-course\06_vm_linux\output.tf for details.
...
/*
resource "storage_image_reference" "my_storage_ref"{
    publisher = var.os.publisher
    offer     = var.os.offer
    sku       = var.os.sku
    version   = var.os.version
...
resource "storage_os_disk" "my_os_disk"{
    name              = "stvm${var.servername}os"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = lookup(var.managed_disk_type, var.location, "Standard_LRS")
