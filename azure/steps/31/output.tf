...
output "virtual_network_name_list_one_input" {
  ...
  #for each value of azurerm_virtual_network.myvnet assigned to vnet
}
...
output "virtual_network_name_keys_function" {
  ...
  #keys function will return just the keys in the map
}
...
output "virtual_network_name_values_function" {
  ...
  #values function will return just the values in map
}
